...
#define INIT_COM_PORT(base) ((struct com_port){ \
    .io_port_base = base, \
    .status = COM_PORT_STATUS_BUSY \
})

extern struct com_port com_ports[COM_PORT_COUNT];
...
int com_port_setup(struct com_port*, uint16_t divisor);

/*  Send a single character out of a given com port. */
void com_send(struct com_port, char c);

/*  Send an entire null terminated string out of a given com port. */
void com_sends(struct com_port, char *s);
